...
angular.module("pamm").directive("gaugeChart", [function () {
    ...
    vm.render = function (chartArea) {
        var size = vm.chart.size ? vm.chart.size : 200;
        var rangeLimit = 130, range = [-rangeLimit, rangeLimit]; // Angle range in degrees
        ...
        d3.select(chartArea)
            ...
            .call(function (g) {
                ...
                (function drawRAG() {
                    ...
                    g.selectAll("g")
                        .data(rag.yellowTo != null && rag.redTo != null ? [
                            {to: domain[1], color: "#2e7d32"},
                            {to: rag.yellowTo, color: "#ef6c00"},
                            {to: rag.redTo, color: "#b21f24"}
                        ] : [{to: domain[1], color: "gray"}])
                        .enter().append("svg:path")
                        .attr("d", function (d) {
                            var xEnd = Math.cos((angle(d.to) - 90) / 180 * Math.PI) * r;
                            var yEnd = Math.sin((angle(d.to) - 90) / 180 * Math.PI) * r;
                            var arcOptions = (d.to - domain[0]) < ((domain[1] - domain[0]) * (rangeLimit / 180)) ? " 0 0,1 " : " 0 1,1 ";
                ...
                (function addScaleAndLabel() {
                    ...
                    g.append("svg:text")
                        .style("fill", "#505050")
                        .style("font-size", size * 0.07 + "pt")
                        .style("font-weight", "bold")
                        .attr("x", 0).attr("y", radius - (size * 0.08))
                    ...
                    g.append("svg:text")
                        ...
                        .style("font-size", size * 0.07 + "pt")
                        .style("font-weight", "bold")
                        .attr("x", 0).attr("y", -(radius - (size * 0.32)))
                        .attr("text-anchor", "middle")
                        .text(unit);
